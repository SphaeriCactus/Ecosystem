class MousePopulation extends Population {
	constructor(number, mutationRate, foodIsAnimal, foodArray, enemyArray) {
		...
		for (let i = 0; i < number; i ++) {
			this.population.push(new Mouse(createVector(random(width), random(height)), new MouseDNA(), this));
		}
	...
	showInfo() {
		fill(0);
		textSize(20);
		text("Mouse Clock: " + floor(this.clock), 100, 50);
	}
	...
	selection() {
		...
		for (let i = 0; i < len; i ++) {
			...
			let child = new Mouse(parentA.pos.copy(), childDNA, this);
			...
			if (random(10) < (parentA.fitness + parentB.fitness)/2) {
				...
				let child2 = new Mouse(parentB.pos.copy(), child2DNA, this);
	...
	crossover(dnaA, dnaB) {
		...
		let newDna = new MouseDNA(newGenes);
		return newDna;
	}
}
